...
const FilterSidebar = () => {
  ...
  const handleColorClick = (colors) => {
    ...
    if (newFilters.colors?.includes(colors)) {
      // Remove color
      newFilters.colors = newFilters.colors.filter((c) => c !== colors);
    } else {
      // Add color
      newFilters.colors = [...(newFilters.colors || []), colors];
    }
  ...
  return (
    <div className="p-4">
          ...
          {allcolors.map((colors) => (
            <button
              ...
              className={`w-8 h-8 rounded-full border border-gray-300 cursor-pointer transition hover:scale-105 ${
                filters.colors.includes(colors) ? "ring-2 ring-blue-500" : ""
              }`}
        ...
        {allsizes.map((sizes) => (
          <div key={sizes} className="flex items-center mb-1">
            <input
              ...
              value={sizes}
              onChange={handleFilterChange}
              checked={filters.sizes.includes(sizes)}
              name="sizes"
              className="mr-2 h-4 w-4 text-blue-500 focus:ring-blue-400 border-gray-300"
            />
      ...
      {/* Price range filter */}
      <div className="mb-8">
        <label className="blcok text-gray-600 font-medium mb-2 ">
          Price Range
        </label>
        <input
          type="range"
